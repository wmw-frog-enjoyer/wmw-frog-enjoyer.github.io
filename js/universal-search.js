(() => {
    ...
    const DATASETS = [
        ...
        },
        {
            ...
            filterGroups: [
                { key: "pattern", label: "patterns", values: item => Array.isArray(item.pattern) ? item.pattern : item.pattern ? [item.pattern] : [] },
                { key: "rating", label: "ratings", values: item => [item.rating] },
                { key: "mapper", label: "mappers", values: item => [item.mapper] },
                { key: "archiver", label: "archivers", values: item => [item.archiver] },
            ],
            searchable: item => [item.name, item.mapper, item.archiver, item.rating, item.id, ...(Array.isArray(item.pattern) ? item.pattern : [item.pattern])],
            date: item => item.date,
            id: item => Number(item.id) || 0,
            render: renderMap,
        },
        {
            ...
            filterGroups: [
                { key: "type", label: "types", values: item => [item.type] },
                { key: "author", label: "authors", values: item => [item.author] },
                { key: "tags", label: "tags", values: item => item.tags || [] },
            ],
            searchable: item => [item.title, item.author, item.type, item.id, ...(item.tags || [])],
            date: item => item.date,
            id: item => Number(item.id) || 0,
            render: renderSkin,
        },
    ...
    function filterValues(dataset, group) {
        return [...new Set(dataset.items.flatMap(group.values).filter(Boolean))].sort((a, b) => String(a).localeCompare(String(b)));
    }
    ...
    function matchesFilters(dataset, item) {
        return dataset.filterGroups.every(group => {
            const values = group.values(item);
            const included = filterValues(dataset, group).filter(value => getState(dataset.key, group.key, value) === "include");
            const excluded = filterValues(dataset, group).filter(value => getState(dataset.key, group.key, value) === "exclude");
            if (values.some(value => excluded.includes(value))) return false;
            return included.length === 0 || included.every(value => values.includes(value));
        });
    }
    ...
    function renderResults() {
        ...
        const groups = state.datasets.filter(dataset => state.enabledDatasets.get(dataset.key) !== false).map(dataset => {
            const results = dataset.items
                ...
                .sort((a, b) => {
                    const dateDiff = String(dataset.date(b) || "").localeCompare(String(dataset.date(a) || ""));
                    return dateDiff || dataset.id(b) - dataset.id(a);
                });
        ...
        if (!groups.length) {
            container.innerHTML = '<p class="universal-search-empty">no results found.</p>';
            return;
        }

        container.innerHTML = groups.map(({ dataset, results }) => `
            <section class="universal-search-result-group" aria-labelledby="search-group-${dataset.key}">
                <div class="universal-search-result-heading">
                    <h2 id="search-group-${dataset.key}"><a href="${escapeHtml(dataset.page)}">${escapeHtml(dataset.label)}</a></h2>
                    <span class="universal-search-result-count">${results.length} result${results.length === 1 ? "" : "s"}</span>
                </div>
                ${results.map(item => dataset.render(item)).join("")}
            </section>
        `).join("");
    ...
    function cardShell(item, meta, body, actions = "", preview = "") {
        return `
            <article class="universal-search-card">
                <div class="card-top-row">${meta}</div>
                <h3 class="universal-search-card-title">${body.title}</h3>
                <div class="universal-search-card-meta">${body.meta || ""}</div>
                ${preview ? `<div class="universal-search-preview">${preview}</div>` : ""}
                ${actions ? `<div class="universal-search-card-actions">${actions}</div>` : ""}
            </article>
        `;
    }
    ...
    function renderArticle(item) {
        return cardShell(item,
            ...
            `<a href="/articles/reader.html?post=${encodeURIComponent(item.id)}">open article</a>`,
        );
    }
    ...
    function renderMap(item) {
        ...
        return cardShell(item,
            `<span class="card-number">#${escapeHtml(item.id)}</span><span class="card-dot">·</span><span class="card-date">${escapeHtml(item.date)}</span>${patterns.map(p => `<span class="card-tag">${escapeHtml(p)}</span>`).join("")}`,
            { title: escapeHtml(item.name), meta: `mapped by ${escapeHtml(item.mapper)} · ${escapeHtml(item.rating)} · archived by ${escapeHtml(item.archiver)}` },
            `<a href="${escapeHtml(item.link)}" target="_blank" rel="noopener noreferrer" download>download map</a>`,
        );
    }

    function renderSkin(item) {
        const typeLabels = {
            "ss-skin": "ss skin", "ss-cursor": "ss cursor", "ssp-cursor": "ssp cursor",
            "ssp-hit": "ssp hit", "ssp-miss": "ssp miss", "ssp-mesh": "ssp mesh",
        };
        let preview = "";
        if (item.type === "ss-skin") {
            const youtube = getYouTubeData(item.video);
            if (youtube) preview = `<iframe src="${youtube.embedUrl}" loading="lazy" allowfullscreen title="${escapeHtml(item.title)} preview"></iframe>`;
        } else if (["ss-cursor", "ssp-cursor"].includes(item.type) && item.file) {
            const previewMap = {"002.png":"002.webp","003.png":"003.webp","006.png":"006-preview.webp","007.png":"007-preview.webp","008.png":"008-preview.webp"};
            preview = `<img src="/ss-hub/skins/assets/${escapeHtml(previewMap[item.file] || item.file)}" alt="${escapeHtml(item.title)} preview" loading="lazy">`;
        } else if (["ssp-hit", "ssp-miss"].includes(item.type)) {
            preview = `<audio controls src="/ss-hub/skins/assets/${escapeHtml(item.file)}"></audio>`;
        }
        let actions = "";
        if (item.type === "ss-skin") actions = `<button type="button" data-copy-file="/ss-hub/skins/assets/${escapeHtml(item.id)}.json">copy skin</button>`;
        else if (item.type === "ss-cursor") actions = `<button type="button" data-copy-text="${escapeHtml(item.code || "")}">copy cursor</button>`;
        else if (item.file) actions = `<a href="/ss-hub/skins/assets/${escapeHtml(item.file)}" download="${escapeHtml(item.title)}">download</a>`;
        return cardShell(item,
            `<span class="card-number">#${escapeHtml(item.id)}</span><span class="card-dot">·</span><span class="card-date">${escapeHtml(item.date)}</span><span class="card-tag">${escapeHtml(typeLabels[item.type] || item.type)}</span>`,
            { title: escapeHtml(item.title), meta: `${escapeHtml(item.author)}${item.tags?.length ? ` · ${renderTags(item.tags)}` : ""}` },
            actions, preview,
        );
    }
    ...
    function bindActions(container) {
        ...
        container.querySelectorAll("[data-copy-file]").forEach(button => {
            button.addEventListener("click", async () => {
                try {
    ...
    async function load() {
        const results = await Promise.all(DATASETS.map(async dataset => {
            try {
                ...
                if (!response.ok) throw new Error(`Could not load ${dataset.source}`);
                return { ...dataset, items: await response.json() };
            } catch (error) {
                console.error(error);
        ...
        document.getElementById("universal-search-input").addEventListener("input", event => {
